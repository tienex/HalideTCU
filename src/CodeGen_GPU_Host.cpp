...
private:
  ...
  void visit(const TensorOp *op) override {
    found_shared = true;
  ...
  void visit(const Allocate *allocate) override {
    ...
    if (allocate->name == "__shared") {
      internal_assert(allocate->type == UInt(8) &&
                      allocate->extents.size() == 1);
...
void CodeGen_GPU_Host<CodeGen_CPU>::compile_func(
    const LoweredFunc &f, const std::string &simple_name,
  ...
  BasicBlock *init_kernels_bb =
      BasicBlock::Create(*context, "init_kernels", function, post_entry);
  ...
  for (pair<const DeviceAPI, CodeGen_GPU_Dev *> &i : cgdev) {
    ...
    Value *kernel_src_ptr = CodeGen_CPU::create_binary_blob(
        kernel_src,
...
void CodeGen_GPU_Host<CodeGen_CPU>::visit(const For *loop) {
  if (CodeGen_GPU_Dev::is_gpu_var(loop->name)) {
    ...
    debug(2) << "Kernel bounds: (" << bounds.num_threads[0] << ", "
             << bounds.num_threads[1] << ", " << bounds.num_threads[2] << ", "
             << bounds.num_threads[3] << ") threads, (" << bounds.num_blocks[0]
             << ", " << bounds.num_blocks[1] << ", " << bounds.num_blocks[2]
    ...
    Value *null_float_ptr =
        ConstantPointerNull::get(CodeGen_LLVM::f32_t->getPointerTo());
    ...
    if (loop->device_api == DeviceAPI::GLSL) {
      ...
      gpu_num_padded_attributes =
          codegen(Variable::make(Int(32), "glsl.num_padded_attributes"));
      gpu_num_coords_dim0 =
          codegen(Variable::make(Int(32), "glsl.num_coords_dim0"));
      gpu_num_coords_dim1 =
          codegen(Variable::make(Int(32), "glsl.num_coords_dim1"));

      // Look up the allocation for the vertex buffer and cast it to the
      // right type
      gpu_vertex_buffer =
          codegen(Variable::make(type_of<float *>(), "glsl.vertex_buffer"));
      gpu_vertex_buffer = builder->CreatePointerCast(
          gpu_vertex_buffer, CodeGen_LLVM::f32_t->getPointerTo());
    }
    ...
    for (size_t i = 0; i < closure_args.size(); i++) {
      if (closure_args[i].is_buffer &&
          allocations.contains(closure_args[i].name)) {
        closure_args[i].size =
            allocations.get(closure_args[i].name).constant_bytes;
      }
    }
    ...
    Value *entry_name_str =
        builder->CreateGlobalStringPtr(kernel_name, "entry_name");
    ...
    Value *gpu_args_arr = create_alloca_at_entry(
        gpu_args_arr_type, num_args + 1, false, kernel_name + "_args");

    // nullptr-terminated list of size_t's
    llvm::Type *gpu_arg_sizes_arr_type =
        ArrayType::get(target_size_t_type, num_args + 1);
    llvm::ArrayType *gpu_arg_types_arr_type =
        ArrayType::get(type_t_type, num_args + 1);
    ...
    if (!runtime_run_takes_types) {
      gpu_arg_sizes_arr =
          create_alloca_at_entry(gpu_arg_sizes_arr_type, num_args + 1, false,
                                 kernel_name + "_arg_sizes");
    }

    llvm::Type *gpu_arg_is_buffer_arr_type = ArrayType::get(i8_t, num_args + 1);
    Value *gpu_arg_is_buffer_arr =
        create_alloca_at_entry(gpu_arg_is_buffer_arr_type, num_args + 1, false,
    ...
    for (int i = 0; i < num_args; i++) {
      ...
      if (!closure_args[i].is_buffer) {
        ...
        // the gpu args array.
        Value *ptr =
            create_alloca_at_entry(val->getType(), 1, false, name + ".stack");
      ...
      builder->CreateStore(bits, builder->CreateConstGEP2_32(
                                     gpu_args_arr_type, gpu_args_arr, 0, i));
      ...
      if (runtime_run_takes_types) {
        ...
        arg_types_array_entries.push_back(
            ConstantStruct::get(type_t_type, arg_type_fields));
      } else {
        // store the size of the argument.
        int size_bytes =
            (closure_args[i].is_buffer) ? 8 : closure_args[i].type.bytes();
        builder->CreateStore(ConstantInt::get(target_size_t_type, size_bytes),
                             builder->CreateConstGEP2_32(gpu_arg_sizes_arr_type,
                                                         gpu_arg_sizes_arr, 0,
                                                         i));
      }

      builder->CreateStore(
          ConstantInt::get(i8_t, closure_args[i].is_buffer),
          builder->CreateConstGEP2_32(gpu_arg_is_buffer_arr_type,
                                      gpu_arg_is_buffer_arr, 0, i));
    }
    // nullptr-terminate the lists
    builder->CreateStore(ConstantPointerNull::get(arg_t),
                         builder->CreateConstGEP2_32(
                             gpu_args_arr_type, gpu_args_arr, 0, num_args));
    if (runtime_run_takes_types) {
      Constant *arg_type_fields[] = {ConstantInt::get(i8_t, 0),
    ...
    internal_assert(is_one(bounds.num_threads[3]) &&
                    is_one(bounds.num_blocks[3]))
        << bounds.num_threads[3] << ", " << bounds.num_blocks[3] << "\n";
    debug(4) << "CodeGen_GPU_Host get_user_context returned "
             << get_user_context() << "\n";
    ...
    Value *launch_args[] = {
        ...
        runtime_run_takes_types
            ? ConstantExpr::getInBoundsGetElementPtr(
                  gpu_arg_types_arr_type, arg_types_array_storage, zeros)
            : builder->CreateConstGEP2_32(
                  gpu_arg_sizes_arr_type, gpu_arg_sizes_arr, 0, 0,
                  "gpu_arg_sizes_ar_ref" + api_unique_name),
        builder->CreateConstGEP2_32(gpu_args_arr_type, gpu_args_arr, 0, 0,
                                    "gpu_args_arr_ref" + api_unique_name),
        builder->CreateConstGEP2_32(gpu_arg_is_buffer_arr_type,
                                    gpu_arg_is_buffer_arr, 0, 0,
    ...
    CodeGen_CPU::create_assertion(
        did_succeed,
        // Should have already called halide_error inside the gpu runtime
        halide_error_code_device_run_failed, result);
  } else {
...
Value *CodeGen_GPU_Host<CodeGen_CPU>::get_module_state(
    const std::string &api_unique_name, bool create) {
