...
std::unique_ptr<llvm::Module> codegen_llvm(const Module &module,
  ...
  std::unique_ptr<Internal::CodeGen_LLVM> cg(
      Internal::CodeGen_LLVM::new_for_target(module.target(), context));
...
#define InitializeAsmParser(target) LLVMInitialize##target##AsmParser();

#define InitializeAsmPrinter(target) LLVMInitialize##target##AsmPrinter();
...
#define InitializeLoongArchTarget() InitializeTarget(LoongArch)
#define InitializeLoongArchAsmParser() InitializeAsmParser(LoongArch)
#define InitializeLoongArchAsmPrinter() InitializeAsmPrinter(LoongArch)
#endif
...
#ifdef WITH_VE
#define InitializeVETarget() InitializeTarget(VE)
#define InitializeVEAsmParser() InitializeAsmParser(VE)
#define InitializeVEAsmPrinter() InitializeAsmPrinter(VE)
#endif
...
CodeGen_LLVM::CodeGen_LLVM(Target t)
    : function(nullptr), context(nullptr), builder(nullptr), value(nullptr),
      very_likely_branch(nullptr), default_fp_math_md(nullptr),
      strict_fp_math_md(nullptr), target(t), void_t(nullptr), i1_t(nullptr),
      i8_t(nullptr), i16_t(nullptr), i32_t(nullptr), i64_t(nullptr),
      f16_t(nullptr), f32_t(nullptr), f64_t(nullptr), buffer_t_type(nullptr),
      metadata_t_type(nullptr), argument_t_type(nullptr),
      scalar_value_t_type(nullptr), device_interface_t_type(nullptr),
      pseudostack_slot_t_type(nullptr),

      // Vector types. These need an LLVMContext before they can be initialized.
      i8x8(nullptr), i8x16(nullptr), i8x32(nullptr), i16x4(nullptr),

      i16x16(nullptr), i32x2(nullptr), i32x4(nullptr), i32x8(nullptr),
      i64x2(nullptr), i64x4(nullptr), f32x2(nullptr), f32x4(nullptr),
      f32x8(nullptr), f16x2(nullptr), f64x2(nullptr), f64x4(nullptr),
      ...
      // Bounds of types
      min_i8(Int(8).min()), max_i8(Int(8).max()), max_u8(UInt(8).max()),

      min_i16(Int(16).min()), max_i16(Int(16).max()), max_u16(UInt(16).max()),

      min_i32(Int(32).min()), max_i32(Int(32).max()), max_u32(UInt(32).max()),

      min_i64(Int(64).min()), max_i64(Int(64).max()), max_u64(UInt(64).max()),

      min_f32(Float(32).min()), max_f32(Float(32).max()),

      min_f64(Float(64).min()), max_f64(Float(64).max()),

      inside_atomic_mutex_node(false), emit_atomic_stores(false),
      emit_tensor_ops(false), emit_all_tops(false), emit_load_top(false),
      emit_store_top(false), destructor_block(nullptr),
...
template <typename T>
CodeGen_LLVM *make_codegen(const Target &target, llvm::LLVMContext &context) {
...
CodeGen_LLVM *CodeGen_LLVM::new_for_target(const Target &target,
  ...
  if (target.features_any_of({Target::CUDA, Target::OpenCL, Target::OpenGL,
                              Target::OpenGLCompute, Target::Metal,
                              Target::D3D12Compute})) {
#ifdef WITH_X86
...
#endif
#ifdef WITH_LOONGARCH
    if (target.arch == Target::LOONGARCH) {
      return make_codegen<CodeGen_GPU_Host<CodeGen_LoongArch>>(target, context);
    }
#endif
#ifdef WITH_MIPS
    ...
    if (target.arch == Target::WebAssembly) {
      return make_codegen<CodeGen_GPU_Host<CodeGen_WebAssembly>>(target,
                                                                 context);
    }
#endif
#ifdef WITH_RISCV
...
#endif
#ifdef WITH_VE
    if (target.arch == Target::VE) {
      return make_codegen<CodeGen_GPU_Host<CodeGen_VE>>(target, context);
    }
#endif
  ...
  } else if (target.arch == Target::LOONGARCH) {
    return make_codegen<CodeGen_LoongArch>(target, context);
  ...
  } else if (target.arch == Target::VE) {
    return make_codegen<CodeGen_VE>(target, context);
  }

  user_error << "Unknown target architecture: " << target.to_string() << "\n";
  return nullptr;
}
...
void CodeGen_LLVM::initialize_llvm() {
  ...
  std::call_once(init_llvm_once, []() {
    ...
    if (!args.empty()) {
      ...
      cl::ParseCommandLineOptions((int)(c_arg_vec.size()), &c_arg_vec[0],
                                  "Halide compiler\n");
    }
...
#define LLVM_TARGET(target) Initialize##target##Target();
#include <llvm/Config/Targets.def>
#undef LLVM_TARGET

#define LLVM_ASM_PARSER(target) Initialize##target##AsmParser();
#include <llvm/Config/AsmParsers.def>
#undef LLVM_ASM_PARSER

#define LLVM_ASM_PRINTER(target) Initialize##target##AsmPrinter();
#include <llvm/Config/AsmPrinters.def>
#undef LLVM_ASM_PRINTER
  });
}
...
void CodeGen_LLVM::init_context() {
  ...
  // for Halide. fast_flags.setAllowReciprocal(); Theoretically, setAllowReassoc
  // could be setUnsafeAlgebra for earlier versions, but that turns on all the
  // flags.
...
#define FixedVectorType VectorType
  ...
  i8x8 = FixedVectorType::get(i8_t, 8);
  i8x16 = FixedVectorType::get(i8_t, 16);
  i8x32 = FixedVectorType::get(i8_t, 32);
  i16x4 = FixedVectorType::get(i16_t, 4);
  i16x8 = FixedVectorType::get(i16_t, 8);
  i16x16 = FixedVectorType::get(i16_t, 16);
  i32x2 = FixedVectorType::get(i32_t, 2);
  i32x4 = FixedVectorType::get(i32_t, 4);
  i32x8 = FixedVectorType::get(i32_t, 8);
  i64x2 = FixedVectorType::get(i64_t, 2);
  i64x4 = FixedVectorType::get(i64_t, 4);
  f16x2 = FixedVectorType::get(f16_t, 2);
  f32x2 = FixedVectorType::get(f32_t, 2);
  f32x4 = FixedVectorType::get(f32_t, 4);
  f32x8 = FixedVectorType::get(f32_t, 8);
  f64x2 = FixedVectorType::get(f64_t, 2);
  f64x4 = FixedVectorType::get(f64_t, 4);
  i4_t = FixedVectorType::get(i32_t, 4);
}
...
void CodeGen_LLVM::add_external_code(const Module &halide_module) {
  for (const ExternalCode &code_blob : halide_module.external_code()) {
    if (code_blob.is_for_cpu_target(get_target())) {
      add_bitcode_to_module(context, *module, code_blob.contents(),
                            code_blob.name());
    }
  }
}

CodeGen_LLVM::~CodeGen_LLVM() { delete builder; }
...
bool CodeGen_LLVM::llvm_LoongArch_enabled = false;
...
bool CodeGen_LLVM::llvm_VE_enabled = false;
...
MangledNames get_mangled_names(const std::string &name, LinkageType linkage,
  ...
  if (linkage != LinkageType::Internal &&
    ...
    names.extern_name = cplusplus_function_mangled_name(
        names.simple_name, namespaces, type_of<int>(), mangle_args, target);
    halide_handle_cplusplus_type inner_type(
    ...
    names.argv_name = cplusplus_function_mangled_name(
        names.argv_name, namespaces, type_of<int>(),
        {ExternFuncArgument(make_zero(void_star_star))}, target);
    names.metadata_name = cplusplus_function_mangled_name(
        names.metadata_name, namespaces,
        type_of<const struct halide_filter_metadata_t *>(), {}, target);
  }
  return names;
}
...
llvm::FunctionType *
CodeGen_LLVM::signature_to_type(const ExternSignature &signature) {
  internal_assert(void_t != nullptr && buffer_t_type != nullptr);
  llvm::Type *ret_type = signature.is_void_return()
                             ? void_t
                             : llvm_type_of(upgrade_type_for_argument_passing(
                                   signature.ret_type()));
  ...
  for (const Type &t : signature.arg_types()) {
    if (t == type_of<struct halide_buffer_t *>()) {
      ...
      llvm_arg_types.push_back(
          llvm_type_of(upgrade_type_for_argument_passing(t)));
    }
  }
...
std::unique_ptr<llvm::Module> CodeGen_LLVM::compile_trampolines(
    const Target &target, llvm::LLVMContext &context, const std::string &suffix,
  ...
  for (const std::pair<std::string, ExternSignature> &e : externs) {
    ...
    llvm::Function *callee =
        llvm::Function::Create(fn_type, llvm::Function::ExternalLinkage,
                               callee_name, codegen->module.get());
...
void CodeGen_LLVM::init_codegen(const std::string &name,
  ...
  module->addModuleFlag(llvm::Module::Warning, "halide_use_soft_float_abi",
                        use_soft_float_abi() ? 1 : 0);
  module->addModuleFlag(llvm::Module::Warning, "halide_mcpu",
                        MDString::get(*context, mcpu()));
  module->addModuleFlag(llvm::Module::Warning, "halide_mattrs",
                        MDString::get(*context, mattrs()));
  module->addModuleFlag(llvm::Module::Warning, "halide_use_pic",
                        use_pic() ? 1 : 0);
  module->addModuleFlag(llvm::Module::Warning,
                        "halide_per_instruction_fast_math_flags",
                        any_strict_float);

  // Ensure some types we need are defined
  buffer_t_type = StructType::getTypeByName(module->getContext(), "struct.halide_buffer_t");
  internal_assert(buffer_t_type)
      << "Did not find halide_buffer_t in initial module";

  type_t_type = StructType::getTypeByName(module->getContext(), "struct.halide_type_t");
  internal_assert(type_t_type)
      << "Did not find halide_type_t in initial module";

  dimension_t_type = StructType::getTypeByName(module->getContext(), "struct.halide_dimension_t");
  internal_assert(dimension_t_type)
      << "Did not find halide_dimension_t in initial module";

  metadata_t_type = StructType::getTypeByName(module->getContext(), "struct.halide_filter_metadata_t");
  internal_assert(metadata_t_type)
      << "Did not find halide_filter_metadata_t in initial module";

  argument_t_type = StructType::getTypeByName(module->getContext(), "struct.halide_filter_argument_t");
  internal_assert(argument_t_type)
      << "Did not find halide_filter_argument_t in initial module";

  scalar_value_t_type = StructType::getTypeByName(module->getContext(), "struct.halide_scalar_value_t");
  internal_assert(scalar_value_t_type)
      << "Did not find halide_scalar_value_t in initial module";

  device_interface_t_type =
      StructType::getTypeByName(module->getContext(), "struct.halide_device_interface_t");
  internal_assert(device_interface_t_type)
      << "Did not find halide_device_interface_t in initial module";

  pseudostack_slot_t_type =
      StructType::getTypeByName(module->getContext(), "struct.halide_pseudostack_slot_t");
  internal_assert(pseudostack_slot_t_type)
      << "Did not find halide_pseudostack_slot_t in initial module";

  semaphore_t_type = StructType::getTypeByName(module->getContext(), "struct.halide_semaphore_t");
  internal_assert(semaphore_t_type)
      << "Did not find halide_semaphore_t in initial module";

  semaphore_acquire_t_type =
      StructType::getTypeByName(module->getContext(), "struct.halide_semaphore_acquire_t");
  internal_assert(semaphore_acquire_t_type)
      << "Did not find halide_semaphore_acquire_t in initial module";

  parallel_task_t_type = StructType::getTypeByName(module->getContext(), "struct.halide_parallel_task_t");
  internal_assert(parallel_task_t_type)
      << "Did not find halide_parallel_task_t in initial module";
}
...
std::unique_ptr<llvm::Module> CodeGen_LLVM::compile(const Module &input) {
  ...
  internal_assert(module && context && builder)
      << "The CodeGen_LLVM subclass should have made an initial module before "
         "calling CodeGen_LLVM::compile\n";
  ...
  for (const auto &f : input.functions()) {
    ...
    // If the Func is externally visible, also create the argv wrapper and
    // metadata. (useful for calling from JIT and other machine interfaces).
    if (f.linkage == LinkageType::ExternalPlusMetadata) {
      llvm::Function *wrapper = add_argv_wrapper(function, names.argv_name);
      llvm::Function *metadata_getter =
          embed_metadata_getter(names.metadata_name, names.simple_name, f.args,
                                input.get_metadata_name_map());
...
void CodeGen_LLVM::begin_func(LinkageType linkage, const std::string &name,
  ...
  for (size_t i = 0; i < args.size(); i++) {
    if (args[i].is_buffer()) {
      ...
      arg_types[i] =
          llvm_type_of(upgrade_type_for_argument_passing(args[i].type));
    }
  }
  ...
  if (!function) {
    function = llvm::Function::Create(func_t, llvm_linkage(linkage),
                                      extern_name, module.get());
  } else {
    ...
    if (func_t != function->getFunctionType()) {
      ...
      user_error << "Cannot create a function with a declaration of mismatched "
                    "type.\n";
    }
  }
    ...
    for (auto &arg : function->args()) {
      if (args[i].is_buffer()) {
        ...
        if (args[i].type != passed_type) {
          llvm::Value *a =
              builder->CreateBitCast(&arg, llvm_type_of(args[i].type));
...
void CodeGen_LLVM::compile_func(const LoweredFunc &f,
  ...
  // If building with MSAN, ensure that calls to
  // halide_msan_annotate_buffer_is_initialized() happen for every output buffer
  // if the function succeeds.
  if (f.linkage != LinkageType::Internal && target.has_feature(Target::MSAN)) {
    llvm::Function *annotate_buffer_fn = module->getFunction(
        "halide_msan_annotate_buffer_is_initialized_as_destructor");
    internal_assert(annotate_buffer_fn)
        << "Could not find "
           "halide_msan_annotate_buffer_is_initialized_as_destructor in "
           "module\n";
    annotate_buffer_fn->addParamAttr(0, Attribute::NoAlias);
    for (const auto &arg : f.args) {
      if (arg.kind == Argument::OutputBuffer) {
        register_destructor(annotate_buffer_fn, sym_get(arg.name + ".buffer"),
                            OnSuccess);
      }
    }
  }
...
// Given a range of iterators of constant ints, get a corresponding vector of
// llvm::Constant.
...
BasicBlock *CodeGen_LLVM::get_destructor_block() {
  if (!destructor_block) {
    ...
    destructor_block =
        BasicBlock::Create(*context, "destructor_block", function);
...
Value *CodeGen_LLVM::register_destructor(llvm::Function *destructor_fn,
                                         Value *obj, DestructorType when) {
  ...
  internal_assert(error_code)
      << "The destructor block is supposed to start with a phi node\n";
...
void CodeGen_LLVM::trigger_destructor(llvm::Function *destructor_fn,
                                      Value *stack_slot) {
  llvm::Function *call_destructor = module->getFunction("call_destructor");
  internal_assert(call_destructor);
  internal_assert(destructor_fn);
  stack_slot = builder->CreatePointerCast(stack_slot,
                                          i8_t->getPointerTo()->getPointerTo());
...
void CodeGen_LLVM::compile_buffer(const Buffer<> &buf) {
  ...
  user_assert(buf.data()) << "Can't embed buffer " << buf.name()
                          << " because it has a null host pointer.\n";
  user_assert(!buf.device_dirty())
      << "Can't embed Image \"" << buf.name() << "\""
      << " because it has a dirty device pointer\n";

  Constant *type_fields[] = {ConstantInt::get(i8_t, buf.type().code()),
  ...
  if (buf.dimensions()) {
    ...
    vector<char> shape_blob((char *)buf.raw_buffer()->dim,
                            (char *)buf.raw_buffer()->dim + shape_size);
    shape = create_binary_blob(shape_blob, buf.name() + ".shape");
    shape =
        ConstantExpr::getPointerCast(shape, dimension_t_type->getPointerTo());
  } else {
  ...
  vector<char> data_blob((const char *)buf.data(),
                         (const char *)buf.data() + buf.size_in_bytes());

  Constant *fields[] = {
      ConstantInt::get(i64_t, 0), // device
      ConstantPointerNull::get(
          device_interface_t_type->getPointerTo()), // device_interface
  ...
  GlobalVariable *global = new GlobalVariable(*module, buffer_t_type, false,
  ...
  Constant *global_ptr =
      ConstantExpr::getInBoundsGetElementPtr(buffer_t_type, global, zero);
...
Constant *CodeGen_LLVM::embed_constant_scalar_value_t(Expr e) {
  ...
  internal_assert(!e.type().is_handle())
      << "Should never see Handle types here.";
  ...
  internal_assert((uint64_t)constant->getType()->getPrimitiveSizeInBits() ==
                  (uint64_t)e.type().bits());
  ...
  // unions directly, so we'll fake it by making a constant array of the
  // elements we need, setting the first to the constant we want, and setting
  // the rest to all-zeros. (This happens to work because
  // sizeof(halide_scalar_value_t) is evenly divisible by
  // sizeof(any-union-field.)

  const size_t value_size = e.type().bytes();
  internal_assert(value_size > 0 &&
                  value_size <= sizeof(halide_scalar_value_t));
  ...
  vector<Constant *> array_entries(array_size,
                                   Constant::getNullValue(constant->getType()));
  array_entries[0] = constant;

  llvm::ArrayType *array_type = ArrayType::get(constant->getType(), array_size);
  GlobalVariable *storage =
      new GlobalVariable(*module, array_type,
...
#if LLVM_VERSION >= 100
  storage->setAlignment(Align(sizeof(halide_scalar_value_t)));
#else
...
Constant *CodeGen_LLVM::embed_constant_expr(Expr e, llvm::Type *t) {
  ...
  if (!is_const(e)) {
    ...
    internal_assert(is_const(e))
        << "Should only see constant values for estimates.";
  }
  ...
  GlobalVariable *storage = new GlobalVariable(
      *module, constant->getType(),
      /*isConstant*/ true, GlobalValue::PrivateLinkage, constant);

  Constant *zero[] = {ConstantInt::get(i32_t, 0)};
  return ConstantExpr::getBitCast(ConstantExpr::getInBoundsGetElementPtr(
                                      constant->getType(), storage, zero),
                                  t->getPointerTo());
}
...
llvm::Function *CodeGen_LLVM::add_argv_wrapper(llvm::Function *fn,
  ...
  llvm::FunctionType *wrapper_func_t =
      llvm::FunctionType::get(wrapper_result_type, wrapper_args_t, false);
  llvm::Function *wrapper_func = llvm::Function::Create(
      wrapper_func_t, llvm::GlobalValue::ExternalLinkage, name, module.get());
  llvm::BasicBlock *wrapper_block =
      llvm::BasicBlock::Create(module->getContext(), "entry", wrapper_func);
  ...
  for (llvm::Function::arg_iterator i = fn->arg_begin(); i != fn->arg_end();
       i++) {
    // Get the address of the nth argument
#if LLVM_VERSION >= 130
    llvm::Value *ptr =
        builder->CreateConstGEP1_32(i8_t->getPointerTo(), arg_array, wrapper_args.size());
    ptr = builder->CreateLoad(i8_t->getPointerTo(), ptr);
#else
    llvm::Value *ptr =
        builder->CreateConstGEP1_32(arg_array, wrapper_args.size());
    ptr = builder->CreateLoad(ptr);
#endif
    if (i->getType() == buffer_t_type->getPointerTo()) {
      // Cast the argument to a buffer_t *
      wrapper_args.push_back(
          builder->CreatePointerCast(ptr, buffer_t_type->getPointerTo()));
    } else {
  ...
  if (result_in_argv) {
#if LLVM_VERSION >= 130
    llvm::Value *result_in_argv_ptr =
        builder->CreateConstGEP1_32(fn->getReturnType(), arg_array, wrapper_args.size());
#else
    llvm::Value *result_in_argv_ptr =
        builder->CreateConstGEP1_32(arg_array, wrapper_args.size());
#endif
...
#if LLVM_VERSION >= 130
      result_in_argv_ptr = builder->CreateLoad(fn->getReturnType(), result_in_argv_ptr);
#else
      result_in_argv_ptr = builder->CreateLoad(result_in_argv_ptr);
#endif
      // Cast to the appropriate type and store
      result_in_argv_ptr = builder->CreatePointerCast(
          result_in_argv_ptr, fn->getReturnType()->getPointerTo());
...
llvm::Function *CodeGen_LLVM::embed_metadata_getter(
    const std::string &metadata_name, const std::string &function_name,
    const std::vector<LoweredArgument> &args,
  ...
  auto map_string =
      [&metadata_name_map](const std::string &from) -> std::string {
  ...
  for (int arg = 0; arg < num_args; ++arg) {
    ...
    StructType *type_t_type = StructType::getTypeByName(module->getContext(), "struct.halide_type_t");
    internal_assert(type_t_type) << "Did not find halide_type_t in module.\n";

    Constant *type_fields[] = {ConstantInt::get(i8_t, args[arg].type.code()),
    ...
    if (args[arg].type.is_handle()) {
      // Handle values are always emitted into metadata as "undefined",
      // regardless of what sort of Expr is provided.
    ...
    if (args[arg].is_buffer() && !argument_estimates.buffer_estimates.empty()) {
      internal_assert((int)argument_estimates.buffer_estimates.size() ==
                      args[arg].dimensions);
      ...
      for (const auto &be : argument_estimates.buffer_estimates) {
        ...
        buffer_estimates_array_entries.push_back(
            embed_constant_expr(min, i64_t));
        buffer_estimates_array_entries.push_back(
            embed_constant_expr(extent, i64_t));
      }

      llvm::ArrayType *buffer_estimates_array = ArrayType::get(
          i64_t->getPointerTo(), buffer_estimates_array_entries.size());
      GlobalVariable *buffer_estimates_array_storage = new GlobalVariable(
          *module, buffer_estimates_array,
          /*isConstant*/ true, GlobalValue::PrivateLinkage,
          ConstantArray::get(buffer_estimates_array,
                             buffer_estimates_array_entries));

      Value *zeros[] = {zero, zero};
      buffer_estimates_array_ptr = ConstantExpr::getInBoundsGetElementPtr(
          buffer_estimates_array, buffer_estimates_array_storage, zeros);
    } else {
      buffer_estimates_array_ptr =
          Constant::getNullValue(i64_t->getPointerTo()->getPointerTo());
    }
    ...
    Constant *argument_fields[] = {
        ...
        buffer_estimates_array_ptr};
    arguments_array_entries.push_back(
        ConstantStruct::get(argument_t_type, argument_fields));
  }
  ...
  GlobalVariable *arguments_array_storage = new GlobalVariable(
      *module, arguments_array,
      /*isConstant*/ true, GlobalValue::PrivateLinkage,
      ConstantArray::get(arguments_array, arguments_array_entries));

  Constant *version =
      ConstantInt::get(i32_t, halide_filter_metadata_t::VERSION);
  ...
  Constant *metadata_fields[] = {
      ...
      /* arguments */
      ConstantExpr::getInBoundsGetElementPtr(arguments_array,
                                             arguments_array_storage, zeros),
      /* target */ create_string_constant(map_string(target.to_string())),
      /* name */ create_string_constant(map_string(function_name))};

  GlobalVariable *metadata_storage =
      new GlobalVariable(*module, metadata_t_type,
                         /*isConstant*/ true, GlobalValue::PrivateLinkage,
                         ConstantStruct::get(metadata_t_type, metadata_fields),
                         metadata_name + "_storage");

  llvm::FunctionType *func_t =
      llvm::FunctionType::get(metadata_t_type->getPointerTo(), false);
  llvm::Function *metadata_getter = llvm::Function::Create(
      func_t, llvm::GlobalValue::ExternalLinkage, metadata_name, module.get());
  llvm::BasicBlock *block =
      llvm::BasicBlock::Create(module->getContext(), "entry", metadata_getter);
  builder->SetInsertPoint(block);
...
void CodeGen_LLVM::optimize_module() {
  ...
  // to *not* enabling these optimizations (and removing the DisableLLVMLoopOpt
  // feature). See https://github.com/halide/Halide/issues/4113 for more info.
  ...
  const bool do_loop_opt =
      !target.has_feature(Target::DisableLLVMLoopOpt) ||
  ...
  pto.SLPVectorization = do_loop_opt;
  ...
  // TODO pto.ForgetAllSCEVInLoopUnroll = true;
  pto.MergeFunctions = do_loop_opt;
  ...
  if (target.has_feature(Target::ASAN)) {
      llvm::AddressSanitizerOptions asan_options;
      asan_options.CompileKernel = false; // always false for user code
      asan_options.Recover = false; // -fsanitize-recover, always false here
      asan_options.UseAfterScope =
          false; // enable -fsanitize-address-use-after-scope?
      constexpr bool use_globals_gc =
          false; // Should ASan use GC-friendly instrumentation for globals?
      constexpr bool use_odr_indicator = true;
      pb.registerPipelineStartEPCallback(
          [asan_options](llvm::ModulePassManager &mpm,
                         llvm::OptimizationLevel level) {
              mpm.addPass(llvm::ModuleAddressSanitizerPass(
                  asan_options, use_globals_gc, use_odr_indicator));
          });
  }
  ...
  if (target.has_feature(Target::ASAN) || target.has_feature(Target::TSAN)) {
      for (llvm::Module::iterator i = module->begin(); i != module->end(); i++) {
          if (target.has_feature(Target::ASAN)) {
              i->addFnAttr(llvm::Attribute::SanitizeAddress);
          }
          if (target.has_feature(Target::TSAN)) {
#if LLVM_VERSION >= 160
#define startswith starts_with
              ...
              if (!(i->getName().startswith("_ZN6Halide7Runtime8Internal15Synchronization") ||
                  ...
                  i->addFnAttr(llvm::Attribute::SanitizeThread);
              }
          }
#if LLVM_VERSION >= 160
#undef startswith
  ...
  b.SLPVectorize = true; // Note: SLP vectorization has no analogue in the
                         // Halide scheduling model
#if LLVM_VERSION >= 90
  // Clear ScEv info for all loops. Certain Halide applications spend a very
  // long time compiling in forgetLoop, and prefer to forget everything
  // and rebuild SCEV (aka "Scalar Evolution") from scratch.
  // Sample difference in compile time reduction at the time of this change was
  // 21.04 -> 14.78 using current ToT release build. (See also
  // https://reviews.llvm.org/rL358304)
  ...
  if (target.has_feature(Target::ASAN)) {
    auto addAddressSanitizerPasses = [](const PassManagerBuilder &builder,
      ...
      constexpr bool use_after_scope =
          false; // enable -fsanitize-address-use-after-scope?
      constexpr bool use_globals_gc =
          false; // Should ASan use GC-friendly instrumentation for globals?

      pm.add(createAddressSanitizerFunctionPass(compile_kernel, recover,
                                                use_after_scope));
#if LLVM_VERSION >= 90
      pm.add(createModuleAddressSanitizerLegacyPassPass(compile_kernel, recover,
                                                        use_globals_gc));
#else
      pm.add(createAddressSanitizerModulePass(compile_kernel, recover,
                                              use_globals_gc));
#endif
    };
    b.addExtension(PassManagerBuilder::EP_OptimizerLast,
                   addAddressSanitizerPasses);
    b.addExtension(PassManagerBuilder::EP_EnabledOnOptLevel0,
                   addAddressSanitizerPasses);
  }

  if (target.has_feature(Target::TSAN)) {
    auto addThreadSanitizerPass = [](const PassManagerBuilder &builder,
                                     legacy::PassManagerBase &pm) {
  ...
  for (llvm::Module::iterator i = module->begin(); i != module->end(); i++) {
    ...
    if (target.has_feature(Target::TSAN)) {
      // Do not annotate any of Halide's low-level synchronization code as it
      // has tsan interface calls to mark its behavior and is much faster if it
      // is not analyzed instruction by instruction.
      if (!(i->getName().startswith(
                "_ZN6Halide7Runtime8Internal15Synchronization") ||
            // TODO: this is a benign data race that re-initializes the detected
            // features; we should really fix it properly inside the
            // implementation, rather than disabling it here as a band-aid.
            i->getName().startswith("halide_default_can_use_target_features") ||
            i->getName().startswith("halide_mutex_") ||
            i->getName().startswith("halide_cond_"))) {
...
void CodeGen_LLVM::sym_pop(const string &name) { symbol_table.pop(name); }

llvm::Value *CodeGen_LLVM::sym_get(const string &name,
                                   bool must_succeed) const {
...
Value *CodeGen_LLVM::codegen(Expr e) {
  ...
  internal_assert(value)
      << "Codegen of an expr did not produce an llvm value\n";
  ...
  // in the long run we should improve the smarts. See
  // https://github.com/halide/Halide/issues/4194.
  ...
  // const bool is_prefetch = e.as<Call>() &&
  // e.as<Call>()->is_intrinsic(Call::prefetch); const bool is_mma =
  // e.as<Call>()
  // &&(e.as<Call>()->is_intrinsic(Call::mma_load)
  // ||e.as<Call>()->is_intrinsic(Call::mma_operation) ||
  // e.as<Call>()->is_intrinsic(Call::mma_store) || e.as<Store>()||e.as<Load>())
  // ;
...
void CodeGen_LLVM::visit(const FloatImm *op) {
  if (op->type.is_bfloat()) {
    codegen(reinterpret(BFloat(16),
                        make_const(UInt(16), bfloat16_t(op->value).to_bits())));
  } else if (op->type.bits() == 16) {
    codegen(reinterpret(Float(16),
                        make_const(UInt(16), float16_t(op->value).to_bits())));
  } else {
...
void CodeGen_LLVM::visit(const Cast *op) {
  ...
  if (upgrade_type_for_arithmetic(src) != src ||
    ...
    if ((src.is_float() && src.bits() < 32) ||
      ...
      internal_error << "Cast from type: " << src << " to " << dst
                     << " unimplemented\n";
    }
    return;
  }
...
void CodeGen_LLVM::visit(const Variable *op) { value = sym_get(op->name); }
...
void CodeGen_LLVM::visit(const Div *op) {
  user_assert(!is_zero(op->b))
      << "Division by constant zero in expression: " << Expr(op) << "\n";
...
void CodeGen_LLVM::visit(const Min *op) {
  ...
  value = codegen(
      Let::make(a_name, op->a, Let::make(b_name, op->b, select(a < b, a, b))));
}
...
Value *CodeGen_LLVM::codegen_buffer_pointer(string buffer, Halide::Type type,
                                            Expr index) {
...
Value *CodeGen_LLVM::codegen_buffer_pointer(Value *base_address,
                                            Halide::Type type, Expr index) {
...
#if LLVM_VERSION >= 130
  return builder->CreateInBoundsGEP(llvm_type_of(type), base_address, index);
#else
...
void CodeGen_LLVM::add_tbaa_metadata(llvm::Instruction *inst, string buffer,
                                     Expr index) {
...
void CodeGen_LLVM::visit(const Load *op) {
  ...
  if (op->type != storage_type) {
    codegen(reinterpret(op->type,
                        Load::make(storage_type, op->name, op->index, op->image,
  ...
  if (op->type.is_scalar()) {
...
#if LLVM_VERSION >= 120
    LoadInst *load = builder->CreateAlignedLoad(llvm_type_of(op->type), ptr, Align(op->type.bytes()));
#else
    ...
    if (ramp && stride && stride->value == 1) {
      ...
      Expr flipped_index =
          Ramp::make(flipped_base, flipped_stride, ramp->lanes);
      ModulusRemainder align = op->alignment;
      // Switch to the alignment of the last lane
      align = align - (ramp->lanes - 1);
      Expr flipped_load =
          Load::make(op->type, op->name, flipped_index, op->image, op->param,
                     op->predicate, align);
      ...
      Value *ptr =
          codegen_buffer_pointer(op->name, op->type.element_of(), ramp->base);
      Value *stride = codegen(ramp->stride);
      value = UndefValue::get(llvm_type_of(op->type));
      for (int i = 0; i < ramp->lanes; i++) {
        Value *lane = ConstantInt::get(i32_t, i);
#if LLVM_VERSION >= 130
        LoadInst *val = builder->CreateLoad(llvm_type_of(op->type.element_of()), ptr);
#else
...
#if LLVM_VERSION >= 130
        ptr = builder->CreateInBoundsGEP(llvm_type_of(op->type.element_of()), ptr, stride);
#else
      ...
      for (int i = 0; i < op->type.lanes(); i++) {
        ...
        Value *ptr =
            codegen_buffer_pointer(op->name, op->type.element_of(), idx);
#if LLVM_VERSION >= 130
        LoadInst *val = builder->CreateLoad(llvm_type_of(op->type.element_of()), ptr);
#else
        LoadInst *val = builder->CreateLoad(ptr);
        ...
        add_tbaa_metadata(val, op->name, op->index);
        vec =
            builder->CreateInsertElement(vec, val, ConstantInt::get(i32_t, i));
      }
      value = vec;
    } else {
      // General gathers
      ...
      for (int i = 0; i < op->type.lanes(); i++) {
        Value *idx =
            builder->CreateExtractElement(index, ConstantInt::get(i32_t, i));
        Value *ptr =
            codegen_buffer_pointer(op->name, op->type.element_of(), idx);
#if LLVM_VERSION >= 130
        LoadInst *val = builder->CreateLoad(llvm_type_of(op->type.element_of()), ptr);
...
void CodeGen_LLVM::visit(const Ramp *op) {
  if (is_const(op->stride) && !is_const(op->base)) {
    ...
    // Otherwise we generate element by element by adding the stride to the base
    // repeatedly
    ...
    for (int i = 0; i < op->type.lanes(); i++) {
      ...
      value =
          builder->CreateInsertElement(value, base, ConstantInt::get(i32_t, i));
    }
  }
}
...
#if LLVM_VERSION >= 120
  Constant *undef = UndefValue::get(FixedVectorType::get(v->getType(), lanes));
#else
...
#if LLVM_VERSION >= 120
  Constant *zeros = ConstantVector::getSplat(ElementCount::getFixed(lanes), zero);
#else
...
Value *CodeGen_LLVM::interleave_vectors(const std::vector<Value *> &vecs) {
  ...
  int vec_elements = llvmx::getVectorNumElements(vecs[0]->getType());
    ...
    if (last) {
      ...
      for (int i = 0, idx = 0; i < result_elements; i++) {
        if (i % vecs.size() < vecs.size() - 1) {
          indices[i] = idx % 2 == 0 ? idx / 2
                                    : idx / 2 + vec_elements * even_vecs.size();
          idx++;
        }
      }
...
void CodeGen_LLVM::scalarize(Expr e) {
  ...
  for (int i = 0; i < e.type().lanes(); i++) {
    ...
    result =
        builder->CreateInsertElement(result, v, ConstantInt::get(i32_t, i));
  }
  value = result;
}
...
void CodeGen_LLVM::codegen_predicated_vector_store(const Store *op) {
  ...
  if (ramp && is_one(ramp->stride)) { // Dense vector store
    ...
    bool is_external =
        (external_buffer.find(op->name) != external_buffer.end());
    ...
    while ((mod_rem.remainder & 1) == 0 && (mod_rem.modulus & 1) == 0 &&
    ...
    // is aligned to at least the native vector width. However, we may be able
    // to do better than just assuming that it is unaligned.
    ...
    for (int i = 0; i < store_lanes; i += native_lanes) {
      ...
      Expr slice_index =
          slice_lanes == 1 ? slice_base
                           : Ramp::make(slice_base, slice_stride, slice_lanes);
      Value *slice_val = slice_vector(val, i, slice_lanes);
      Value *elt_ptr =
          codegen_buffer_pointer(op->name, value_type.element_of(), slice_base);
      Value *vec_ptr = builder->CreatePointerCast(
          elt_ptr, slice_val->getType()->getPointerTo());

      Value *slice_mask = slice_vector(vpred, i, slice_lanes);
#if LLVM_VERSION >= 120
      Instruction *store_inst =
          builder->CreateMaskedStore(slice_val, vec_ptr, Align(alignment), slice_mask);
#else
      Instruction *store_inst =
...
#if LLVM_VERSION >= 120
      builder->CreateAlignedStore(v, ptr, Align(value_type.bytes()));
#else
...
Value *CodeGen_LLVM::codegen_dense_vector_load(const Load *load, Value *vpred) {
  ...
  internal_assert(ramp && is_one(ramp->stride))
      << "Should be dense vector load\n";

  bool is_external =
      (external_buffer.find(load->name) != external_buffer.end());
  ...
  if (is_external) {
    if (load->param.defined()) {
      ...
      // If we're JITting, use the actual pointer value to determine alignment
      // for embedded buffers.
      alignment = gcd(alignment, (int)(((uintptr_t)load->image.data()) &
  ...
  for (int i = 0; i < load_lanes; i += native_lanes) {
    ...
    Expr slice_base = simplify(ramp->base + i);
    Expr slice_stride = make_one(slice_base.type());
    Expr slice_index = slice_lanes == 1
                           ? slice_base
                           : Ramp::make(slice_base, slice_stride, slice_lanes);
#if LLVM_VERSION >= 120
    llvm::Type *slice_type =
        FixedVectorType::get(llvm_type_of(load->type.element_of()), slice_lanes);
#else
    llvm::Type *slice_type =
        VectorType::get(llvm_type_of(load->type.element_of()), slice_lanes);
#endif
    Value *elt_ptr =
        codegen_buffer_pointer(load->name, load->type.element_of(), slice_base);
    Value *vec_ptr =
        builder->CreatePointerCast(elt_ptr, slice_type->getPointerTo());

    Instruction *load_inst;
    if (vpred != nullptr) {
      Value *slice_mask = slice_vector(vpred, i, slice_lanes);
#if LLVM_VERSION >= 130
      load_inst = builder->CreateMaskedLoad(slice_type, vec_ptr, Align(alignment), slice_mask);
...
#if LLVM_VERSION >= 120
      load_inst = builder->CreateAlignedLoad(slice_type, vec_ptr, Align(alignment));
#else
...
void CodeGen_LLVM::codegen_predicated_vector_load(const Load *op) {
  ...
  if (ramp && is_one(ramp->stride)) { // Dense vector load
    ...
    Expr flipped_load =
        Load::make(op->type, op->name, flipped_index, op->image, op->param,
                   const_true(op->type.lanes()), align);
    ...
    Expr load_expr =
        Load::make(op->type, op->name, op->index, op->image, op->param,
                   const_true(op->type.lanes()), op->alignment);
    debug(4) << "Scalarize predicated vector load\n\t" << load_expr << "\n";
    Expr pred_load =
        Call::make(load_expr.type(), Call::if_then_else,
...
void CodeGen_LLVM::codegen_atomic_store(const Store *op) {
  ...
  user_assert(is_one(op->predicate))
      << "Atomic predicated store is not supported.\n";
  ...
  // store. For llvm version < 9, the atomicRMW operations only support integers
  // so we also check that.
  Expr equiv_load = Load::make(value_type, op->name, op->index, Buffer<>(),
                               op->param, op->predicate, op->alignment);
  Expr delta =
      simplify(common_subexpression_elimination(op->value - equiv_load));
  bool is_atomic_add =
      supports_atomic_add(value_type) && !expr_uses_var(delta, op->name);
  if (is_atomic_add) {
    ...
    if (value_type.is_scalar()) {
      ...
      if (value_type.is_float()) {
        builder->CreateAtomicRMW(AtomicRMWInst::FAdd, ptr, val,
#if LLVM_VERSION >= 130
                                 Align(op->value.type().bits()),
#endif
                                 AtomicOrdering::Monotonic);
      } else {
        builder->CreateAtomicRMW(AtomicRMWInst::Add, ptr, val,
#if LLVM_VERSION >= 130
                                 Align(op->value.type().bits()),
#endif
                                 AtomicOrdering::Monotonic);
      ...
      for (int i = 0; i < value_type.lanes(); i++) {
        ...
        Value *ptr =
            codegen_buffer_pointer(op->name, value_type.element_of(), idx);
#if LLVM_VERSION >= 90
        if (value_type.is_float()) {
          builder->CreateAtomicRMW(AtomicRMWInst::FAdd, ptr, v,
#if LLVM_VERSION >= 130
                                   Align(op->value.type().bits()),
#endif
                                   AtomicOrdering::Monotonic);
        } else {
          builder->CreateAtomicRMW(AtomicRMWInst::Add, ptr, v,
#if LLVM_VERSION >= 130
                                   Align(op->value.type().bits()),
#endif
                                   AtomicOrdering::Monotonic);
    ...
    for (int lane_id = 0; lane_id < value_type.lanes(); lane_id++) {
      ...
      BasicBlock *loop_bb = BasicBlock::Create(ctx, "casloop.start", f);
      // Load the old value for compare and swap test.
      Value *ptr = nullptr;
      if (value_type.is_scalar()) {
        ptr = codegen_buffer_pointer(op->name, value_type, op->index);
      } else {
        Value *idx = builder->CreateExtractElement(
            vec_index, ConstantInt::get(i32_t, lane_id));
        ptr = codegen_buffer_pointer(op->name, value_type.element_of(), idx);
      }
#if LLVM_VERSION >= 120
      LoadInst *orig = builder->CreateAlignedLoad(llvm_type_of(op->value.type()), ptr, Align(value_type.bytes()));
#else
...
#if LLVM_VERSION >= 140
      PHINode *cmp =
          builder->CreatePHI(llvm_type_of(op->value.type()), 2, "loaded");
#else
      PHINode *cmp =
          builder->CreatePHI(ptr_type->getPointerElementType(), 2, "loaded");
#endif
      Value *cmp_val = cmp;
      ...
      if (need_bit_cast) {
        IntegerType *int_type =
            builder->getIntNTy(val_type->getPrimitiveSizeInBits());
      ...
      Value *cmpxchg_pair = builder->CreateAtomicCmpXchg(
          ptr, cmp_val, val,
#if LLVM_VERSION >= 130
          Align(val->getType()->getPrimitiveSizeInBits()),
#endif
          AtomicOrdering::Monotonic,
          AtomicOrdering::Monotonic);
      Value *val_loaded =
          builder->CreateExtractValue(cmpxchg_pair, 0, "val_loaded");
      ...
      BasicBlock *exit_bb = BasicBlock::Create(ctx, "casloop.end", f);
...
void CodeGen_LLVM::visit(const Call *op) {
  ...
  if (op->is_intrinsic(Call::debug_to_file)) {
    ...
    internal_assert(debug_to_file)
        << "Could not find halide_debug_to_file function in initial module\n";
    ...
    buffer = builder->CreatePointerCast(
        buffer, debug_to_file->getFunctionType()->getParamType(3));
    ...
    // Check if an appropriate vector abs for this type exists in the initial
    // module
    ...
    if (t.is_vector() && builtin_absd) {
      ...
      codegen(Let::make(a_name, op->args[0],
                        Let::make(b_name, op->args[1],
                                  Select::make(a_var < b_var, b_var - a_var,
                                               a_var - b_var))));
    }
    ...
    Expr e = lower_lerp(cast(t, op->args[0]), cast(t, op->args[1]),
                        cast(wt, op->args[2]));
    e = cast(op->type, e);
    codegen(e);
    ...
    llvm::Function *fn =
        Intrinsic::getDeclaration(module.get(), Intrinsic::ctpop, arg_type);
    Value *a = codegen(op->args[0]);
    ...
    std::vector<llvm::Type *> arg_type(1);
    arg_type[0] = llvm_type_of(op->args[0].type());
    llvm::Function *fn = Intrinsic::getDeclaration(
        module.get(),
    ...
    if (op->type.is_vector()) {
      ...
      value = ConstantPointerNull::get(
          dyn_cast<PointerType>(llvm_type_of(op->type)));
    } else {
      ...
      if (args.size() == 1) {
        ...
        for (size_t i = 0; i < args.size(); i++) {
          Value *elem_ptr =
              builder->CreateConstInBoundsGEP2_32(aggregate_t, value, 0, i);
    ...
    if (op->type.is_vector()) {
      ...
      for (size_t i = 0; i < op->args.size(); i++) {
        ...
        if (op->args[i].as<StringImm>()) {
          ...
          // Not a strict upper bound (there isn't one), but ought to be enough
          // for most buffers.
          buf_size += 512;
        } else {
      ...
      llvm::Value *dst = buf;
#if LLVM_VERSION >= 130
      llvm::Value *buf_end = builder->CreateConstGEP1_32(i8_t, buf, buf_size);
#else
      llvm::Value *buf_end = builder->CreateConstGEP1_32(buf, buf_size);
#endif

      llvm::Function *append_string =
          module->getFunction("halide_string_to_string");
      llvm::Function *append_int64 =
          module->getFunction("halide_int64_to_string");
      llvm::Function *append_uint64 =
          module->getFunction("halide_uint64_to_string");
      llvm::Function *append_double =
          module->getFunction("halide_double_to_string");
      llvm::Function *append_pointer =
          module->getFunction("halide_pointer_to_string");
      llvm::Function *append_buffer =
          module->getFunction("halide_buffer_to_string");
      ...
      for (size_t i = 0; i < op->args.size(); i++) {
        ...
        if (s) {
          ...
          buf = builder->CreatePointerCast(
              buf, append_buffer->getFunctionType()->getParamType(2));
    ...
    if (op->type == type_of<struct halide_buffer_t *>() && call &&
        call->is_intrinsic(Call::size_of_halide_buffer_t)) {
      ...
      if (op->type == type_of<struct halide_dimension_t *>()) {
        value = create_alloca_at_entry(dimension_t_type,
                                       *sz / sizeof(halide_dimension_t));
      } else {
    ...
    if (!f) {
      ...
      f = llvm::Function::Create(func_t, llvm::Function::ExternalLinkage,
                                 fn->value, module.get());
    ...
    for (size_t i = 0; i < op->args.size(); i += 2) {
      ...
      if (!sub_fn) {
        extern_sub_fn_name =
            get_mangled_names(sub_fn_name, LinkageType::External,
                              NameMangling::Default, current_function_args,
                              get_target())
                .extern_name;
        ...
        llvm::Type *result_type =
            llvm_type_of(upgrade_type_for_argument_passing(op->type));
      ...
      if (!sub_fn_ptr) {
        debug(1) << "Did not find function ptr " << extern_sub_fn_name
                 << ", assuming extern \"C\".\n";
        sub_fn_ptr = new GlobalVariable(
            *module, sub_fn->getType(),
    ...
    const string global_name =
        unique_name(base_fn->getName().str() + "_indirect_fn_ptr");
    GlobalVariable *global = new GlobalVariable(
        *module, base_fn->getType(),
        /*isConstant*/ false, GlobalValue::PrivateLinkage,
        ConstantPointerNull::get(base_fn->getType()), global_name);
#if LLVM_VERSION >= 130
    ...
    BasicBlock *global_inited_bb =
        BasicBlock::Create(*context, "global_inited_bb", function);
    BasicBlock *global_not_inited_bb =
        BasicBlock::Create(*context, "global_not_inited_bb", function);
    BasicBlock *call_fn_bb =
        BasicBlock::Create(*context, "call_fn_bb", function);

    // Only init the global if not already inited.
    //
    // Note that we deliberately do not attempt to make this threadsafe via
    // (e.g.) mutexes; the requirements of the conditions above mean that
    // multiple writes *should* only be able to re-write the same value, which
    // is harmless for our purposes, and avoiding such code simplifies and
    // speeds the resulting code.
    //
    // (Note that if we ever need to add a way to clear the cached function
    // pointer, we may need to reconsider this, to avoid amusingly horrible race
    // conditions.)
    builder->CreateCondBr(builder->CreateIsNotNull(loaded_value),
                          global_inited_bb, global_not_inited_bb,
                          very_likely_branch);
    ...
    for (int i = sub_fns.size() - 1; i >= 0; i--) {
      ...
      if (!selected_value) {
        ...
        selected_value =
            builder->CreateSelect(c, sub_fn.fn_ptr, selected_value);
      }
    }
    ...
    llvm::CallInst *call =
        builder->CreateCall(base_fn->getFunctionType(), phi, call_args);
    value = call;
    ...
    user_error
        << "Signed integer overflow occurred during constant-folding. Signed"
           " integer overflow for int32 and int64 is undefined behavior in"
           " Halide.\n";
  } else if (op->is_intrinsic(Call::indeterminate_expression)) {
    user_error
        << "Indeterminate expression occurred during constant-folding.\n";
  } else if (op->is_intrinsic(Call::quiet_div)) {
    internal_assert(op->args.size() == 2);
    if (is_zero(op->args[1])) {
      value = UndefValue::get(llvm_type_of(op->type));
    } else {
      Expr equiv = Call::make(
          op->type, Call::if_then_else,
          {op->args[1] == 0, undef(op->type), op->args[0] / op->args[1]},
          Call::Intrinsic);
      equiv.accept(this);
    }
    ...
    IRBuilder<llvm::ConstantFolder,
              llvm::IRBuilderDefaultInserter>::FastMathFlagGuard
        guard(*builder);
    ...
    Expr x = op->args[0];
    Expr y = op->args[1];
    Halide::Expr abs_x_pow_y =
        Internal::halide_exp(Internal::halide_log(abs(x)) * y);
    Halide::Expr nan_expr =
        Call::make(x.type(), "nan_f32", {}, Call::PureExtern);
    Expr iy = floor(y);
    ...
    // isfinite(e) -> (fabs(e) != infinity && !isnan(e)) -> (fabs(e) != infinity
    // && e == e)
    Expr e = op->args[0];
    internal_assert(e.type().is_float());
    Expr inf = e.type().max();
    ...
    if (op->call_type == Call::ExternCPlusPlus) {
      user_assert(get_target().has_feature(Target::CPlusPlusMangling))
          << "Target must specify C++ name mangling "
             "(\"c_plus_plus_name_mangling\") in order to call C++ externs. ("
          << op->name << ")\n";
      ...
      name = cplusplus_function_mangled_name(name, namespaces, op->type,
                                             mangle_args, get_target());
    } else {
      name = op->name;
    }
    ...
    if (takes_user_context) {
      internal_assert(fn)
          << "External function " << op->name
          << " is marked as taking user_context, but is not in the runtime "
             "module. Check if runtime_api.cpp needs to be rebuilt.\n";
    ...
    if (!fn) {
      ...
      FunctionType *func_t =
          FunctionType::get(scalar_result_type, arg_types, false);

      fn = llvm::Function::Create(func_t, llvm::Function::ExternalLinkage, name,
                                  module.get());
      fn->setCallingConv(CallingConv::C);
      debug(4) << "Did not find " << op->name
               << ". Declared it extern \"C\".\n";
    } else {
      ...
      for (size_t i = takes_user_context ? 1 : 0;
           i < std::min(args.size(), (size_t)(func_t->getNumParams())); i++) {
        ...
        if (halide_arg.type().is_handle()) {
          ...
          if (halide_arg.type().is_vector() && !t->isVectorTy()) {
            t = FixedVectorType::get(t, halide_arg.type().lanes());
          }
    ...
    if (op->type.is_scalar()) {
      ...
      if (vec_fn) {
        value = call_intrin(llvm_type_of(op->type), w, vec_fn->getName().str(), args);
      } else {
...
Constant *CodeGen_LLVM::create_binary_blob(const vector<char> &data,
                                           const string &name, bool constant) {
  internal_assert(!data.empty());
  llvm::Type *type = ArrayType::get(i8_t, data.size());
  GlobalVariable *global = new GlobalVariable(
      *module, type, constant, GlobalValue::PrivateLinkage, 0, name);
  ArrayRef<unsigned char> data_array((const unsigned char *)&data[0],
                                     data.size());
...
#if LLVM_VERSION >= 100
  global->setAlignment(Align(alignment));
#else
...
void CodeGen_LLVM::create_assertion(Value *cond, Expr message,
  ...
  if (vt) {
    Value *scalar_cond =
        builder->CreateExtractElement(cond, ConstantInt::get(i32_t, 0));
    for (unsigned i = 1; i < llvmx::getVectorNumElements(vt); i++) {
      Value *lane =
          builder->CreateExtractElement(cond, ConstantInt::get(i32_t, i));
  ...
  BasicBlock *assert_fails_bb =
      BasicBlock::Create(*context, "assert failed", function);
  BasicBlock *assert_succeeds_bb =
      BasicBlock::Create(*context, "assert succeeded", function);

  // If the condition fails, enter the assert body, otherwise, enter the block
  // after
...
void CodeGen_LLVM::return_with_error_code(llvm::Value *error_code) {
  ...
  // Hook up our error code to the phi node that the destructor block starts
  // with.
...
void CodeGen_LLVM::visit(const For *op) {
  ...
  if (op->for_type == ForType::Parallel ||
      (op->for_type == ForType::Serial && acquire &&
    ...
    BasicBlock *loop_bb =
        BasicBlock::Create(*context, std::string("for ") + op->name, function);
    // Create the block that comes after the loop
    BasicBlock *after_bb = BasicBlock::Create(
        *context, std::string("end for ") + op->name, function);
    ...
    internal_error << "Unknown type of For node. Only Serial and Parallel For "
                      "nodes should survive down to codegen.\n";
  }
}
// SAME AS FOR except step and cond not upper bound
void CodeGen_LLVM::visit(const DoWhile *op) {
  Value *min = codegen(op->min);
  ...
  if (op->for_type == ForType::Parallel ||
    ...
    BasicBlock *after_bb = BasicBlock::Create(
        *context, std::string("end for ") + op->name, function);
    BasicBlock *true_bb = BasicBlock::Create(*context, "true_bb", function);
    Expr break_cond = op->cond;
    Value *step_value = codegen(op->step);
    builder->CreateBr(loop_bb);
    builder->SetInsertPoint(loop_bb);
    ...
    // Within the loop, the variable is equal to the phi value
    sym_push(op->name, phi);
    Value *loop_cond = codegen(break_cond);
    // maybe exit the lloop
    builder->CreateCondBr(loop_cond, after_bb, true_bb);
    builder->SetInsertPoint(true_bb);
    // Emit the loop body
    codegen(op->body);

    // Update the counter
    Value *next_var = builder->CreateNSWAdd(phi, step_value);
...
void CodeGen_LLVM::do_parallel_tasks(const vector<ParallelTask> &tasks) {
  ...
  // Fill in the closure
  pack_closure(closure_t, closure_ptr, closure, symbol_table, buffer_t_type,
               builder);
  ...
  llvm::Value *task_stack_ptr =
      create_alloca_at_entry(parallel_task_t_type, num_tasks);

  llvm::Type *args_t[] = {i8_t->getPointerTo(), i32_t, i8_t->getPointerTo()};
  FunctionType *task_t = FunctionType::get(i32_t, args_t, false);
  llvm::Type *loop_args_t[] = {i8_t->getPointerTo(), i32_t, i32_t,
                               i8_t->getPointerTo(), i8_t->getPointerTo()};
  ...
  for (int i = 0; i < num_tasks; i++) {
    ...
    class MayBlock : public IRVisitor {
      ...
      void visit(const Acquire *op) override { result = true; }
    ...
    bool use_do_par_for = (num_tasks == 1 && min_threads.result == 0 &&
                           t.semaphores.empty() && !task_parent);

    // Make the array of semaphore acquisitions this task needs to do before it
    // runs.
    Value *semaphores;
    Value *num_semaphores = ConstantInt::get(i32_t, (int)t.semaphores.size());
    if (!t.semaphores.empty()) {
      semaphores = create_alloca_at_entry(semaphore_acquire_t_type,
                                          (int)t.semaphores.size());
      for (int i = 0; i < (int)t.semaphores.size(); i++) {
        Value *semaphore = codegen(t.semaphores[i].semaphore);
        semaphore = builder->CreatePointerCast(
            semaphore, semaphore_t_type->getPointerTo());
        Value *count = codegen(t.semaphores[i].count);
        Value *slot_ptr = builder->CreateConstGEP2_32(semaphore_acquire_t_type,
                                                      semaphores, i, 0);
        builder->CreateStore(semaphore, slot_ptr);
        slot_ptr = builder->CreateConstGEP2_32(semaphore_acquire_t_type,
                                               semaphores, i, 1);
        builder->CreateStore(count, slot_ptr);
      }
    } else {
      semaphores =
          ConstantPointerNull::get(semaphore_acquire_t_type->getPointerTo());
    }
    ...
    llvm::Value *task_ptr =
        builder->CreatePointerCast(function, fn_type->getPointerTo());
    ...
    if (use_do_par_for) {
      ...
      t.body = For::make(t.loop_var, Variable::make(Int(32), loop_min_name),
                         Variable::make(Int(32), loop_extent_name),
                         ForType::Serial, DeviceAPI::None, t.body);
      ++iter;
    ...
    Value *closure_handle = builder->CreatePointerCast(
        iterator_to_pointer(iter), closure_t->getPointerTo());
    ...
    if (use_do_par_for) {
      ...
      internal_assert(do_par_for)
          << "Could not find halide_do_par_for in initial module\n";
      ...
      Value *slot_ptr = builder->CreateConstGEP2_32(parallel_task_t_type,
                                                    task_stack_ptr, i, 0);
      builder->CreateStore(task_ptr, slot_ptr);
      slot_ptr = builder->CreateConstGEP2_32(parallel_task_t_type,
                                             task_stack_ptr, i, 1);
      ...
      slot_ptr = builder->CreateConstGEP2_32(parallel_task_t_type,
                                             task_stack_ptr, i, 7);
      builder->CreateStore(ConstantInt::get(i32_t, min_threads.result),
                           slot_ptr);
      slot_ptr = builder->CreateConstGEP2_32(parallel_task_t_type,
                                             task_stack_ptr, i, 8);
  ...
  if (!result) {
    llvm::Function *do_parallel_tasks =
        module->getFunction("halide_do_parallel_tasks");
    internal_assert(do_parallel_tasks)
        << "Could not find halide_do_parallel_tasks in initial module\n";
    do_parallel_tasks->addParamAttr(2, Attribute::NoAlias);
    Value *task_parent = sym_get("__task_parent", false);
    if (!task_parent) {
      task_parent = ConstantPointerNull::get(i8_t->getPointerTo()); // void*
    }
    Value *args[] = {get_user_context(), ConstantInt::get(i32_t, num_tasks),
                     task_stack_ptr, task_parent};
...
void CodeGen_LLVM::get_parallel_tasks(Stmt s, vector<ParallelTask> &result,
  ...
  if (const Fork *f = s.as<Fork>()) {
    ...
    result.push_back(ParallelTask{loop->body,
                                  {},
                                  loop->name,
                                  loop->min,
                                  loop->extent,
                                  const_false(),
                                  task_debug_name(prefix)});
  } else if (loop && loop->for_type == ForType::Serial && acquire &&
             !expr_uses_var(acquire->count, loop->name)) {
    const Variable *v = acquire->semaphore.as<Variable>();
    internal_assert(v);
    add_suffix(prefix, ".for." + v->name);
    ParallelTask t{loop->body,
                   {},
                   loop->name,
                   loop->min,
                   loop->extent,
    ...
    result.push_back(
        ParallelTask{s, {}, "", 0, 1, const_false(), task_debug_name(prefix)});
  }
}
...
void CodeGen_LLVM::visit(const Acquire *op) { do_as_parallel_task(op); }

void CodeGen_LLVM::visit(const Fork *op) { do_as_parallel_task(op); }
...
void CodeGen_LLVM::visit(const Store *op) {
  ...
  if (inside_atomic_mutex_node) {
    user_assert(value_type.is_scalar())
        ...
        << " is lowered into a mutex lock, which does not support "
           "vectorization.\n";
  }
  ...
  if (value_type.is_scalar()) {
...
#if LLVM_VERSION >= 120
    StoreInst *store =
        builder->CreateAlignedStore(val, ptr, Align(value_type.bytes()));
#else
    StoreInst *store =
        builder->CreateAlignedStore(val, ptr, value_type.bytes());
#endif
    ...
    if (ramp && is_one(ramp->stride)) {
      ...
      // If it is an external buffer, then we cannot assume that the host
      // pointer is aligned to at least the native vector width. However, we may
      // be able to do better than just assuming that it is unaligned.
      if (is_external && op->param.defined()) {
        int host_alignment = op->param.host_alignment();
        alignment = gcd(alignment, host_alignment);
      }
      ...
      for (int i = 0; i < store_lanes; i += native_lanes) {
        ...
        Expr slice_index =
            slice_lanes == 1
                ...
                : Ramp::make(slice_base, slice_stride, slice_lanes);
        Value *slice_val = slice_vector(val, i, slice_lanes);
        Value *elt_ptr = codegen_buffer_pointer(
            op->name, value_type.element_of(), slice_base);
        Value *vec_ptr = builder->CreatePointerCast(
            elt_ptr, slice_val->getType()->getPointerTo());
#if LLVM_VERSION >= 120
        StoreInst *store =
            builder->CreateAlignedStore(slice_val, vec_ptr, Align(alignment));
#else
        StoreInst *store =
            builder->CreateAlignedStore(slice_val, vec_ptr, alignment);
#endif
      ...
      for (int i = 0; i < ramp->lanes; i++) {
        ...
        if (const_stride) {
...
#if LLVM_VERSION >= 120
          Value *p = builder->CreateConstInBoundsGEP1_32(
              llvm_type_of(ptr_type), ptr, const_stride->value * i);
#else
          Value *p = builder->CreateConstInBoundsGEP1_32(
              ptr, const_stride->value * i);
#endif
          ...
          add_tbaa_metadata(store, op->name, op->index);
#if LLVM_VERSION >= 130
          ptr = builder->CreateInBoundsGEP(llvm_type_of(ptr_type), ptr, stride);
#else
          ptr = builder->CreateInBoundsGEP(ptr, stride);
#endif
        }
...
void CodeGen_LLVM::codegen_asserts(const vector<const AssertStmt *> &asserts) {
  ...
  BasicBlock *no_errors_bb =
      BasicBlock::Create(*context, "no_errors_bb", function);
  ...
  llvm::MDBuilder md_builder(*context);
  llvm::MDNode *switch_very_likely_branch =
      md_builder.createBranchWeights(weights);
  auto *switch_inst =
      builder->CreateSwitch(codegen(case_idx), no_errors_bb, asserts.size(),
                            switch_very_likely_branch);
  for (int i = 0; i < (int)asserts.size(); i++) {
    BasicBlock *fail_bb =
        BasicBlock::Create(*context, "assert_failed", function);
    switch_inst->addCase(ConstantInt::get(IntegerType::get(*context, 32), i),
                         fail_bb);
...
void CodeGen_LLVM::visit(const Block *op) {
  ...
  if (a && is_pure(a->condition)) {
    ...
    asserts.push_back(a);
    Stmt s = op->rest;
    while ((op = s.as<Block>()) && (a = op->first.as<AssertStmt>()) &&
           is_pure(a->condition) && asserts.size() < 63) {
      asserts.push_back(a);
      s = op->rest;
    }
...
void CodeGen_LLVM::visit(const Atomic *op) {
  if (op->mutex_name != "") {
    internal_assert(!inside_atomic_mutex_node)
        << "Nested atomic mutex locks detected. This might causes a "
           "deadlock.\n";
    ScopedValue<bool> old_inside_atomic_mutex_node(inside_atomic_mutex_node,
                                                   true);
    // Mutex locking & unlocking are handled by function calls generated by
    // previous lowering passes.
    codegen(op->body);
  } else {
    // Issue atomic stores.
...
Value *CodeGen_LLVM::create_alloca_at_entry(llvm::Type *t, int n,
  ...
  if (zero_initialize) {
    if (n == 1) {
      ...
      builder->CreateMemSet(ptr, Constant::getNullValue(t), n, Align(align));
    }
  }
  builder->restoreIP(here);
  return ptr;
...
Value *CodeGen_LLVM::call_intrin(const Type &result_type, int intrin_lanes,
  ...
  return call_intrin(llvm_type_of(result_type), intrin_lanes, name, arg_values);
}

Value *CodeGen_LLVM::call_intrin(llvm::Type *result_type, int intrin_lanes,
                                 const string &name,
                                 vector<Value *> arg_values) {
  internal_assert(result_type->isVectorTy())
      << "call_intrin is for vector intrinsics only\n";

  int arg_lanes = int(llvmx::getVectorNumElements(result_type));
    ...
    for (int start = 0; start < arg_lanes; start += intrin_lanes) {
      ...
      for (size_t i = 0; i < arg_values.size(); i++) {
        if (arg_values[i]->getType()->isVectorTy()) {
          int arg_i_lanes =
              int(llvmx::getVectorNumElements(arg_values[i]->getType()));
          ...
          args.push_back(slice_vector(arg_values[i], start * reduce,
                                      intrin_lanes * reduce));
        } else {
      ...
      llvm::Type *result_slice_type =
          llvm::FixedVectorType::get(result_type->getScalarType(), intrin_lanes);

      results.push_back(
          call_intrin(result_slice_type, intrin_lanes, name, args));
    }
  ...
  if (!fn) {
    llvm::Type *intrinsic_result_type =
        FixedVectorType::get(result_type->getScalarType(), intrin_lanes);
    FunctionType *func_t =
        FunctionType::get(intrinsic_result_type, arg_types, false);
    fn = llvm::Function::Create(func_t, llvm::Function::ExternalLinkage, name,
                                module.get());
    fn->setCallingConv(CallingConv::C);
  }
...
Value *CodeGen_LLVM::slice_vector(Value *vec, int start, int size) {
  int vec_lanes = llvmx::getVectorNumElements(vec->getType());
...
Value *CodeGen_LLVM::concat_vectors(const vector<Value *> &v) {
  ...
  while (vecs.size() > 1) {
    ...
    for (size_t i = 0; i < vecs.size() - 1; i += 2) {
      ...
      int w1 = llvmx::getVectorNumElements(v1->getType());
      int w2 = llvmx::getVectorNumElements(v2->getType());
...
Value *CodeGen_LLVM::shuffle_vectors(Value *a, Value *b,
  ...
  for (size_t i = 0; i < llvm_indices.size(); i++) {
    if (indices[i] >= 0) {
      internal_assert(indices[i] <
                      int(llvmx::getVectorNumElements(a->getType())) * 2);
...
Value *CodeGen_LLVM::shuffle_vectors(Value *a,
                                     const std::vector<int> &indices) {
  Value *b = UndefValue::get(a->getType());
  return shuffle_vectors(a, b, indices);
}

std::pair<llvm::Function *, int>
CodeGen_LLVM::find_vector_runtime_function(const std::string &name, int lanes) {
  // Check if a vector version of the function already
  ...
  for (size_t i = 0; i < sizes_to_try.size(); i++) {
    ...
    llvm::Function *vec_fn =
        module->getFunction(name + "x" + std::to_string(l));
    if (vec_fn) {
      return {vec_fn, l};
    }
  }
...
bool CodeGen_LLVM::use_pic() const { return true; }
