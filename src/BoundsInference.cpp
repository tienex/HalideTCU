...
class DependsOnBoundsInference : public IRVisitor {
  ...
  void visit(const Variable *var) override {
    if (ends_with(var->name, ".max") || ends_with(var->name, ".min")) {
      result = true;
    }
  }

  void visit(const Call *op) override {
    if (op->name == Call::buffer_get_min || op->name == Call::buffer_get_max) {
      result = true;
    } else {
      IRVisitor::visit(op);
    }
...
public:
  ...
  struct Stage {
    ...
    // Note that a function definition might have different LHS or reduction
    // domain (if it's an update def) or RHS per specialization. All
    // specializations of an init definition should have the same LHS. This also
    // pushes all the reduction domains it encounters into the 'rvars' set for
    // later use.
    vector<vector<CondValue>> compute_exprs_helper(const Definition &def,
                                                   bool is_update) {
        ...
        for (auto &val : vecs[i]) {
          if (!predicates.empty()) {
            ...
            for (size_t i = 1; i < predicates.size(); ++i) {
              cond_val = Call::make(
                  cond_val.type(), Internal::Call::if_then_else,
      ...
      for (size_t i = specializations.size(); i > 0; i--) {
        ...
        vector<vector<CondValue>> s_result =
        ...
        for (size_t i = 0; i < result.size(); i++) {
          result[i].insert(result[i].end(), s_result[i].begin(),
                           s_result[i].end());
        }
      }
    ...
    void compute_exprs() {
      ...
      if (func.extern_definition_proxy_expr().defined()) {
        exprs.push_back(
            CondValue(const_true(), func.extern_definition_proxy_expr()));
      }
        ...
        Expr visit(const Select *op) override {
          if (is_pure(op->condition)) {
            return Call::make(op->type, Call::if_then_else,
                              {mutate(op->condition), mutate(op->true_value),
    ...
    // Check if the dimension at index 'dim_idx' is always pure (i.e. equal to
    // 'dim') in the definition (including in its specializations)
    bool is_dim_always_pure(const Definition &def, const string &dim,
                            int dim_idx) {
    ...
    bool
    is_fused_with_others(const vector<vector<Function>> &fused_groups,
                         const vector<set<FusedPair>> &fused_pairs_in_groups,
                         const Function &producing_func,
                         int producing_stage_index, string consumer_name,
                         int consumer_stage, string var) {
      ...
      size_t index;
      {
        const auto &iter = std::find_if(
            fused_groups.begin(), fused_groups.end(),
      ...
      const vector<Dim> &dims =
          (producing_stage_index == 0)
              ? producing_func.definition().schedule().dims()
              : producing_func.update(producing_stage_index - 1)
                    .schedule()
                    .dims();
      ...
      {
        const auto &iter =
            std::find_if(dims.begin(), dims.end(), [&var](const Dim &d) {
              return var_name_match(d.var, var);
            });
      ...
      for (auto &pair : fused_pairs_in_groups[index]) {
        if (((pair.func_1 == consumer_name) &&
            ...
            ((pair.func_2 == consumer_name) &&
             ((int)pair.stage_2 == consumer_stage))) {
          const auto &iter =
              std::find_if(dims.begin(), dims.end(), [&pair](const Dim &d) {
                return var_name_match(d.var, pair.var_name);
              });
          internal_assert(iter != dims.end());
    ...
    Stmt define_bounds(Stmt s, Function producing_func,
                       string producing_stage_index,
                       int producing_stage_index_index, string loop_level,
                       const vector<vector<Function>> &fused_groups,
                       const vector<set<FusedPair>> &fused_pairs_in_groups,
      ...
      if (func.has_extern_definition() &&
        ...
        if (!in_pipeline.empty()) {
          ...
          Expr outer_query = Variable::make(type_of<struct halide_buffer_t *>(),
                                            outer_query_name);
          string inner_query_name = func.name() + ".o0.bounds_query";
          Expr inner_query = Variable::make(type_of<struct halide_buffer_t *>(),
                                            inner_query_name);
          for (int i = 0; i < func.dimensions(); i++) {
            ...
            s = LetStmt::make(func.name() + ".s0." + func_args[i] + ".max",
                              new_max, s);
            s = LetStmt::make(func.name() + ".s0." + func_args[i] + ".min",
                              new_min, s);
          }
      ...
      if (in_pipeline.count(name) == 0) {
        ...
        for (size_t i = 0; i < func.schedule().bounds().size(); i++) {
          ...
          if ((i == 0) && (stage > 0) &&
              func.update(stage - 1).schedule().tensor_core()) {
            bound.extent = make_const(Int(32), 68 * 256);
          }
          ...
          if (bound.extent.defined()) {
            ...
            // producers.
            if (bound.min.defined() || compute_at.is_root() ||
                (compute_at.match(loop_level) && store_at.match(loop_level))) {
          ...
          if (bound.modulus.defined()) {
            ...
            max_plus_one =
                ((max_plus_one + bound.modulus - 1) / bound.modulus) *
                bound.modulus;
            max_plus_one += bound.remainder;
      ...
      for (size_t d = 0; d < b.size(); d++) {
        ...
        if (b[d].is_single_point()) {
          s = LetStmt::make(arg + ".min", Variable::make(Int(32), arg + ".max"),
                            s);
        } else {
    ...
    Stmt do_bounds_query(Stmt s, const set<string> &in_pipeline,
      ...
      for (size_t j = 0; j < args.size(); j++) {
        if (args[j].is_expr()) {
          ...
          for (int k = 0; k < input.outputs(); k++) {
            string name = input.name() + ".o" + std::to_string(k) +
                          ".bounds_query." + func.name();
            ...
            bounds_inference_args.push_back(
                Variable::make(type_of<struct halide_buffer_t *>(), name));
          ...
          Expr in_buf = Variable::make(type_of<struct halide_buffer_t *>(),
                                       name + ".buffer");

          // Copy the input buffer into a query buffer to mutate.
          string query_name = name + ".bounds_query." + func.name();

          Expr alloca_size = Call::make(Int(32), Call::size_of_halide_buffer_t,
                                        {}, Call::Intrinsic);
          Expr query_buf =
              Call::make(type_of<struct halide_buffer_t *>(), Call::alloca,
                         {alloca_size}, Call::Intrinsic);
          Expr query_shape = Call::make(
              type_of<struct halide_dimension_t *>(), Call::alloca,
              {(int)(sizeof(halide_dimension_t) * dims)}, Call::Intrinsic);
          query_buf =
              Call::make(type_of<struct halide_buffer_t *>(),
                         Call::buffer_init_from_buffer,
                         {query_buf, query_shape, in_buf}, Call::Extern);

          lets.push_back({query_name, query_buf});
          Expr buf = Variable::make(type_of<struct halide_buffer_t *>(),
                                    query_name, b, p, ReductionDomain());
          bounds_inference_args.push_back(buf);
          // Although we expect ImageParams to be properly initialized and
          // sanitized by the caller, we create a copy with copy_memory (not
          // msan-aware), so we need to annotate it as initialized.
          buffers_to_annotate.push_back(bounds_inference_args.back());
        } else {
      ...
      for (int j = 0; j < func.outputs(); j++) {
        ...
        string buf_name =
            func.name() + ".o" + std::to_string(j) + ".bounds_query";
        bounds_inference_args.push_back(
            Variable::make(type_of<struct halide_buffer_t *>(), buf_name));
        // Since this is a temporary, internal-only buffer used for bounds
        // inference, we need to mark it
        buffers_to_annotate.push_back(bounds_inference_args.back());
      ...
      if (target.has_feature(Target::MSAN)) {
        ...
        for (auto &buffer : buffers_to_annotate) {
          ...
          // the result.
          Expr sizeof_buffer_t = cast<uint64_t>(Call::make(
              Int(32), Call::size_of_halide_buffer_t, {}, Call::Intrinsic));
          Stmt mark_buffer = Evaluate::make(
              Call::make(Int(32), "halide_msan_annotate_memory_is_initialized",
      ...
      Expr e =
          func.make_call_to_extern_definition(bounds_inference_args, target);

      // Check if it succeeded
      string result_name = unique_name('t');
      Expr result = Variable::make(Int(32), result_name);
      Expr error =
          Call::make(Int(32), "halide_error_bounds_inference_call_failed",
    ...
    void populate_scope(Scope<Interval> &result) {
      ...
      /*for (size_t i = 0; i < func.definition().schedule().bounds().size();
      i++) { const Bound &b = func.definition().schedule().bounds()[i];
  ...
  BoundsInference(const vector<Function> &f, const vector<vector<Function>> &fg,
                  const vector<set<FusedPair>> &fp,
                  const vector<Function> &outputs, const FuncValueBounds &fb,
                  const Target &target)
      : funcs(f), fused_groups(fg), fused_pairs_in_groups(fp), func_bounds(fb),
        target(target) {
    ...
    for (size_t i = 0; i < inlined.size(); i++) {
      if (i < f.size() - 1 && f[i].schedule().compute_level().is_inlined() &&
          f[i].can_be_inlined()) {
        inlined[i] = true;
      } else {
        inlined[i] = false;
    ...
    */
    ...
    for (size_t i = 0; i < stages.size(); i++) {
      ...
      if (consumer.func.has_extern_definition() &&
        ...
        const vector<ExternFuncArgument> &args =
            consumer.func.extern_arguments();
        ...
        for (size_t j = 0; j < args.size(); j++) {
          if (args[j].is_func()) {
            ...
            string stage_name =
                f.name() + ".s" + std::to_string(f.updates().size());
            Box b(f.dimensions());
            for (int d = 0; d < f.dimensions(); d++) {
              string buf_name = f.name() + ".o0.bounds_query." + consumer.name;
              Expr buf =
                  Variable::make(type_of<struct halide_buffer_t *>(), buf_name);
              Expr min = Call::make(Int(32), Call::buffer_get_min, {buf, d},
                                    Call::Extern);
              Expr max = Call::make(Int(32), Call::buffer_get_max, {buf, d},
                                    Call::Extern);
        ...
        for (auto &cval : consumer.exprs) {
          ...
          for (auto &i : new_boxes) {
            // Add the condition on which this value is evaluated to the box
            // before merging
            Box &box = i.second;
            box.used = cval.cond;
      ...
      for (size_t j = 0; j < i; j++) {
        ...
        // A consumer depends on *all* stages of a producer, not just the last
        // one.
        ...
        if (!b.empty()) {
          ...
          for (size_t k = 0; k < b.size(); k++) {
            if (!b[k].is_bounded()) {
              ...
              err << " of Function " << consumer.name << " calls function "
                  << producer.name << " in an unbounded way in dimension " << k
                  << "\n";
              user_error << err.str();
            }
          }
    ...
    // The region required of the each output is expanded to include the size of
    // the output buffer.
    for (Function output : outputs) {
      ...
      for (int d = 0; d < output.dimensions(); d++) {
        ...
        Expr min = Variable::make(
            Int(32), buffer_name + ".min." + std::to_string(d), buf);
        Expr extent = Variable::make(
            Int(32), buffer_name + ".extent." + std::to_string(d), buf);
  ...
  Stmt visit(const For *op) override {
    ...
    if (!no_pipelines && producing >= 0 && !f.has_extern_definition()) {
      ...
      box =
          box_provided(body, stages[producing].name, empty_scope, func_bounds);
    ...
    if (!no_pipelines) {
      ...
      // B.2) We're downstream (a consumer) of a func for which we care about
      // the bounds.
      ...
      if (producing >= 0 && !inner_productions.empty()) {
        ...
        for (size_t i = 0; i < box.size(); i++) {
          ...
          if (box[i].is_single_point()) {
            body = LetStmt::make(var + ".max",
                                 Variable::make(Int(32), var + ".min"), body);
          } else {
...
Stmt bounds_inference(Stmt s, const vector<Function> &outputs,
  ...
  s = BoundsInference(funcs, fused_func_groups, fused_pairs_in_groups, outputs,
                      func_bounds, target)
          .mutate(s);
